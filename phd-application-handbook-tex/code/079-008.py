...
def parse_procar_lm_decomposed_autospin(procar_path: str, force_nspin: Optional[int] = None):
    """
    Robust PROCAR parser for "lm decomposed" format.

    Handles:
      - Fortran D/d exponents in numbers (converted to E/e).
      - k-point coordinates that may not be cleanly whitespace-separated.
      - Extra blocks after the (ion ... tot) table (e.g., spin projections in LSORBIT/noncollinear),
        which are safely ignored.

    Returns:
      kpoints: (nk, 3)
      energies: (nk, nb, nspin)
      weights: (nk, nb, nion, norb, nspin), norb=9 (s,py,pz,px,dxy,dyz,dz2,dxz,x2-y2)
    """
    ...
    # Generic number finder: supports D/d and E/e exponents and catches adjacent signs
    num_pat = re.compile(r"[+\-]?\d+(?:\.\d*)?(?:[EeDd][+\-]?\d+)?")
    ...
    # For collinear ISPIN=2 without explicit "spin component" blocks
    seen_k_cycle = 0
    last_k_index = None
    ...
    while i < len(lines):
        ...
        # k-point line
        if ln.lstrip().lower().startswith("k-point"):
            # Extract k-point index (first integer after 'k-point')
            mki = re.search(r"^\s*k-point\s+(\d+)", ln, re.IGNORECASE)
            if mki:
                k_index_1b = int(mki.group(1))
                ...
                # Prefer parsing coords after ':' to avoid grabbing the k-point index
                after = ln.split(":", 1)[1] if ":" in ln else ln
                nums = num_pat.findall(after)
                if len(nums) >= 3 and cur_spin == 0:
                    kpoints[cur_k, :] = [fnum(nums[0]), fnum(nums[1]), fnum(nums[2])]
            i += 1
            continue

        # band energy line
        if ln.lstrip().lower().startswith("band") and "energy" in ln.lower():
            mb = re.search(r"^\s*band\s+(\d+)", ln, re.IGNORECASE)
            if mb:
                cur_b = int(mb.group(1)) - 1
                after = re.split(r"energy", ln, flags=re.IGNORECASE, maxsplit=1)[1]
                nums = num_pat.findall(after)
                if nums:
                    energies[cur_k, cur_b, cur_spin] = fnum(nums[0])
            i += 1
            continue

        # orbital weight table header
        if ion_header_pat.match(ln):
            # Read exactly nion lines after header. Ignore the 'tot' line and any subsequent projection blocks.
            for j in range(nion):
                row = lines[i + 1 + j].split()
                if len(row) < 11:
                    continue
                ion_idx_1b = int(row[0]) - 1
                vals = [float(x) for x in row[1:10]]
                weights[cur_k, cur_b, ion_idx_1b, :, cur_spin] = vals
            i += 1 + nion
            continue
    ...
    if nan_pos.size > 0:
        ex = nan_pos[:8].tolist()
        raise RuntimeError(
            "PROCAR parse incomplete: energies has NaNs (some 'band ... energy ...' lines were not parsed). "
            f"Examples [k_idx0, b_idx0, spin]: {ex}. "
            "If this persists, check that PROCAR is complete (not truncated) and belongs to this KPOINTS."
        )

    return kpoints, energies, weights, nk, nb, nion, nspin, norb
