...
不超过 4 个有效行、总长度不超过 320 字符且符合命令/短代码特征的代码块直接在正文展示；其余长代码抽离到代码附录，并在原位置给出清单编号。正文保留原文说明、公式、表格和图片，每篇文章的信息栏只显示日期和在线原文。`classification.json` 会列出收录文章的分类依据、代码块数、正文短代码数和图片数，便于人工复核。
