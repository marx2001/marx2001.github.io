...
用于记录写脚本的有用的版本，有些暂时用不上，但是绘制n重简并的能带的时候可以用到，还有的脚本的功能是任意数量的轨道组合也能画图，同时指定颜色，这两种脚本只选一种来使用，未免有些可惜，所以都记录在此，后面根据论文排版不同，选择不同版本的脚本进行绘图。

所有脚本的使用方法可能会有细微差别，具体体现在使用方法上。

脚本默认在计算能带的目录下，要有Procar等文件，procar版本是6.5.0，其他版本并没有测试过，所以不知道是否会存在问题，建议本地电脑安装pyprocar，并运行脚本。
...
对全局字体加深要求，全部加粗，且规定字体为Times New Roman。调整刻度线、标题、刻度与轴线之间的距离，使得图片可以用于论文。
...
def main():
    ...
    # ✅ 只保留下/左刻度线，朝内，并把刻度标签与轴线距离设为 pad=14
...
```python
python 3fatband.py --elements Mo --orbitals d --fixed_colors "dxy=#e53935,dyz=#f2a400,dz2=#2f4aa8,dxz=#43a047,x2-y2=#8e24aa" --out Mo_d.png
```

最终效果

<img src="/img/超超交换路径/bg-t4.png" style="width:100%; margin-bottom:5px;">

2026.3.4补充新代码，考虑soc的时候，上面脚本解析会失败，所以下面版本是绘制考虑soc的投影能带图。
