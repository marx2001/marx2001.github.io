...
categories: sci-note
permalink: /sci-note_posts/20251122-Bi-m
---
...
```
NBANDS = 64（该数值应为 CPU 核心数的整数倍；此处使用 64 核提交），并在 INCAR 中加入 LWANNIER90 = T。
...
与公众号的文件不同，很奇怪，但原因应该是我的拟合的轨道不同，我设置的Bi:l=1



一些可能的原因：我的NBANDS设置是64，而示例中是40，导致我的拟合能带和DFT能带有一些差距。这个保持质疑，我始终按照博主给的INCAR进行计算，但是始终得不到和公众号一样的带隙。开
ISPIN会让费米能级升高。加入下面tag后，费米能级继续升高：
...
不用管了，不知道是什么原因，导致的费米能级无法复现，我们只需要知道整体步骤即可：

使用由VASP生成的 wannier90.amn、wannier90.mmn、wannier90.eig和 wannier90.win文件，生成最大局域瓦尼尔函数。

在后台运行Wannier90，命令为：wannier90.x wannier90 &。

这将生成一系列文件，我们所需的是 wannier90.chk（检查点文件）。

使用得到的瓦尼尔函数计算体材料的能带结构，并将其与通过DFT计算得到的能带进行比较。

将步骤1中得到的 wannier90.chk和 wannier90.eig文件复制到当前文件夹，在后台运行Wannier90，命令为：wannier90.x wannier90 &。

执行命令 gnuplot --persist wannier90_band.gnu来绘制能带结构图。

将瓦尼尔方法计算得到的能带结构与DFT计算的结果进行比较。

使用得到的瓦尼尔函数来获取瓦尼尔哈密顿量。

将步骤1中得到的 wannier90.chk和 wannier90.eig文件复制到当前文件夹，在后台运行Wannier90，命令为：wannier90.x wannier90 &。

检查输出文件 wannier90_hr.dat。
...
计算威尔逊环（Wilson loop）和Z2拓扑不变量。

将 wannier90_hr.dat从 5_wannier_cal目录的最后一步复制到此处。

将 wt_Z2.in复制为 wt.in。

执行命令 wt.x &来计算 Z2 拓扑不变量。

执行命令 gnuplot wanniercenter3D_Z2.gnu来查看瓦尼尔电荷中心的演化。

检查 WT.out输出文件和 wanniercenter3D_Z2.eps图像文件。

计算表面态。

将瓦尼尔计算步骤中的 wannier90_hr.dat文件复制到此处。

将 wt_SS.in复制为 wt.in。

执行命令 wt.x &来计算表面态。

执行命令 gnuplot surfdos_r.gnu来绘制表面态图谱。

检查 surfdos_r.png图像文件。


完整的执行流程我已经fork到我的github库内，此处为基本的笔记指导，方便查阅。下方为基本的示例输入文件：
...
### 报错：
----------------------------------------------------------------------------- | _ ____ _ _ _____ _ | | | | | _ \ | | | | / ____| | | | | | | | |_) | | | | | | | __ | | | | |_| | _ < | | | | | | |_ | |_| | | _ | |_) | | |__| | | |__| | _ | | (_) |____/ \____/ \_____| (_) | | | | internal error in: mkpoints_change.F at line: 794 | | | | internal error in GENERATE_KPOINTS_TRANS: number of G-vector | | changed in star 7778 7776 | | | | If you are not a developer, you should not encounter this problem. | | Please submit a bug report. | | | -----------------------------------------------------------------------------
